...
class ItemDatabase
{
	...
	public static var _sfwItemTypes:Array<ItemType> =
	{
		var _sfwTmpItems = [];

		_sfwTmpItems.push( { name:"Hydrogen Peroxide", basePrice:750, shopImage:AssetPaths.hydrogen_peroxide_shop__png, dialog:HeraShopDialog.hydrogenPeroxide, index:ITEM_HYDROGEN_PEROXIDE=_sfwTmpItems.length } );//3
		_sfwTmpItems.push( { name:"Acetone", basePrice:750, shopImage:AssetPaths.acetone_shop__png, dialog:HeraShopDialog.acetone, index:ITEM_ACETONE=_sfwTmpItems.length } );
	...
	public static function refillItems():Void
	{
		...
		// trace (_shopItems.length);
		while (i < _shopItems.length)
		{

			var currentItem:ShopItem = _shopItems[i];
			var itemType:ItemType = currentItem.getItemType();
			
			// trace ("ITEM NAME TEST: " + itemType.name);
			if (itemType.name == "Grey Anal Beads") {
				// This is the specific item you're looking for
				// Do something with currentItem or itemType
			}



			// trace ("Shop item: " + _shopItems[i] + " discount: " +_shopItems[i]._discount);

			// if ( _shopItems[i].getItemType)
			// 	trace ("SHOP ITEM # DETECTED!!1! WEE WOO WEE WOO");
			
			if (_shopItems[i]._discount >= 5 || _shopItems[i]._discount <= -5 )
			{
				// trace (_shopItems[i] + "'s discount is greater than 4 OR less than -4");
			...
			{
				// trace (_shopItems[i] + "'s discount is NOT greater than 4 or less than -4");
				i++;
			}
			// trace ("pruned? " + prunedInventory);
		}
		if (_shopItems.length < 5 && _warehouseItemIndexes.length > 0 && !prunedInventory)
		{
			// trace (" there are less than 5 shop items, AND the warehouse isn't empty, AND the invemtory wasn't pruned");
			_newItems = true;
			addShopItem();
		}
		while (_shopItems.length < 3 && _warehouseItemIndexes.length > 0)
		{
			// trace ("there are less than 3 items in the shop AND the warehouuse isn't empty");
			_newItems = true;
			addShopItem();
		}
...
class ShopItem
{
	...
	{
		// trace("hello itemDatabase!");
		// trace ("THE NAME IS " + ItemDatabase._itemTypes[_itemIndex].name + "\n\n\n");
		// trace ("THE ITEM INDEX IS " + ItemDatabase._itemTypes[_itemIndex] + "\n\n\n");

		// for (i in ItemDatabase._itemTypes)
		// 		trace (i.name);
				
		// if (ItemDatabase._itemTypes[_itemIndex].name == "Grey Anal Beads")
		// {
			// trace ( ItemDatabase._itemTypes[_itemIndex]);
