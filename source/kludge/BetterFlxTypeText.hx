...
/**
 * Adapted from FlxTypeText, with a few workarounds for bugs and shortcomings in HaxeFlixel.
 */
class BetterFlxTypeText extends FlxText
{
	/**
		...
		if (_typing || _erasing)
		{
			...
			if ((_typing && _timer >= delay) || (_erasing && _timer >= eraseDelay))
			{
				...
				if (sounds != null && !useDefaultSound)
				{
					...
					#else
					// workaround for flixel-addons #329; FlxTypeText doesn't play sounds frequently enough for HTML5 targets
					// https://github.com/HaxeFlixel/flixel-addons/issues/389
